...
float3 trace(__constant Sphere* spheres, const Ray* camray, const int sphere_count, const int* seed0, const int* seed1){
	...
	for (int bounces = 0; bounces < 25; bounces++){
		...
		if (!intersect_scene(spheres, &ray, &t, &hitsphere_id, sphere_count))
			return accum_color += mask * (float3)(0.0f, 0.0f, 0.0f);//(float3)(0.15f, 0.15f, 0.25f);
...
__kernel void render(const int width, const int height,
                     __constant struct Sphere* spheres, const int obj_num,
                     __global uchar4* output)   // use unsigned char vector
{
    int x = get_global_id(0);
    int y = get_global_id(1);
    if (x >= width || y >= height) return;

    unsigned int seed0 = (unsigned int)(x*1973u + 12345u);
    unsigned int seed1 = (unsigned int)(y*9277u + 891u);

    int idx = y * width + x;
    struct Ray camray = createCamRay(x, y, width, height);

    float3 col = (float3)(0);
    float invS = 1.0f / (float)SAMPLES;
    for (int i = 0; i < SAMPLES; ++i)
        col += trace(spheres, &camray, obj_num, &seed0, &seed1) * invS;

    // Reinhard tonemap + gamma
    float3 mapped = col / (1.0f + col);
